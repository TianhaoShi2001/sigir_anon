...
def run_a_trail(train_config,log_file=None, save_mode=False,save_dir=None,evaluate_only=False,warm_or_cold=None,
    ...
    if  (config.synthetic_data_path is not None):
        if os.path.exists(config.synthetic_data_path):
            ...
            synthetic_data = synthetic_data[['uid','iid','label']].reset_index(drop=True)
            ...
            if config.influence_save_name:
                ...
                def update_signs(a, a_sign, b_sign, x):
                    diff_indices = np.where(a_sign != b_sign)[0]
                ...
                for iter, influence_iter in enumerate(influence_iters):
                    if iter == 0:
                        ...
                        if_perturb_sign = np.where(if_list < 0, -1, 1)
                        # else:
                        #     if_perturb_sign = np.where(if_list < 0, -1, 1)
                        # if_perturb_sign = update_signs(if_list, if_current_sign,
                        # if_last_sign, max(1,int(config.perturb_ratio*if_list.shape[0])))
                    else:
                        if_list += influence_iter * (config.ensemble_k * iter + 1)
                        if_last_sign = np.copy(if_perturb_sign)
                        if_current_sign = np.where(if_list < 0, -1, 1)
                        if_perturb_sign = update_signs(if_list, if_current_sign,
                            if_last_sign, max(1,int(config.perturb_ratio*if_list.shape[0])))
                
                synthetic_data['if_sign'] = if_perturb_sign 
                synthetic_data['if_score'] = if_list
                if ('sample_ratio' not in train_config.keys()) or (train_config['sample_ratio'] == 0):
                    # quantile_sample = 0
                    synthetic_data['retain'] = synthetic_data['if_sign'] < 0
                else:
                    sorted_indices = synthetic_data.sort_values(by=['if_sign', 'if_score'], ascending=[True, True]).index
                    synthetic_data['retain'] = False
                    sample_num = int(train_config['sample_ratio'] * synthetic_data.shape[0])
                    synthetic_data.loc[sorted_indices[:sample_num], 'retain'] = True

                retain_count = synthetic_data.groupby('iid')['retain'].transform('sum')

                insufficient_items = synthetic_data.loc[retain_count < config.min_item_num, 'iid'].unique()

                for item in insufficient_items:
                    item_data = synthetic_data[synthetic_data['iid'] == item]
                    not_retained = item_data[~item_data['retain']].sort_values(by=['if_sign', 'if_score'], ascending=[True, True])
                    needed = config.min_item_num - item_data['retain'].sum()
                    # 补齐 retain
                    synthetic_data.loc[not_retained.index[:needed], 'retain'] = True
                synthetic_data = synthetic_data[synthetic_data['retain']]
